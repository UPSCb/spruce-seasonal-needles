...
# goi <- goi[goi$id %in% rownames(vst_bio_rep_mean_goi),] # only keeps the goi which matched with the genes in the vst data
# goi <- goi[order(match(goi$id,rownames(vst_bio_rep_mean_goi))),] # order the goi in the same order as in the vst table
...
#' The first one, cluster 2,2 had most of its genes that match to the expression pattern of cluster 1 in photosynthetic goi, and also for some of its genes a tendancy to match cluster 2. Plus, there are a new pattern with peaks during summer.
